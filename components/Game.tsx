...
export default function Game({ SCREEN_HEIGHT, PLAYER_SIZE, PLAYER_X, PLAYER_Y, PIPE_WIDTH, GAP, obstacles, velocityRef, main_character, gameLoop, score, bestScore, jump, endGame, setDisplayGame, setDisplayPauseMenu }: GameProps) {
    ...
    return (
        <div onClick={jump} className="relative w-screen h-screen overflow-hidden">
            ...
            {obstacles.map((obs) => (
                <div key={obs.id}>
                    <motion.div animate={{ scaleX: [1, 1.02, 1] }} className="absolute z-10 bg-linear-to-b from-green-500 to-green-800 border-[5px] border-green-950 rounded-b-[18px] shadow-2xl transition duration-500 cursor-pointer" style={{ left: obs.x, top: 0, width: PIPE_WIDTH, height: obs.topHeight }} />
                    <motion.div animate={{ scaleX: [1, 1.02, 1] }} className="absolute z-10 bg-linear-to-b from-green-500 to-green-800 border-[5px] border-green-950 rounded-t-[18px] shadow-2xl transition duration-500 cursor-pointer" style={{ left: obs.x, top: obs.topHeight + GAP, width: PIPE_WIDTH, height: SCREEN_HEIGHT - (obs.topHeight + GAP) }} />
                </div>
            ))}
